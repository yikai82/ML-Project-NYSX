...
## Note: Fix double mlflow run issue ### 

## Date: 2025-11-17
## Test three random seeds(111,222,333) to assess model with different random weight initializations
...
## 1. Make Sure to check for Run Number and Batch Number 
...
for Tick in Batch_1: 
    output_PATH = f"../../production/TEST/{Tick}"
    model_PATH =f"../../models/TEST"
    os.makedirs(f"{output_PATH}", exist_ok=True)
    os.makedirs(f"{model_PATH}", exist_ok=True)
    
    # Artifact_PATH =f"../../production/TEST/{Tick}"    
    ...
    with mlflow.start_run(run_name=RUN_NAME, experiment_id=exp_id, nested = False):
        ...
        ### >> 2.1 Define a helper function to create a training data as input sequence (the sliding window and look_back = 20)
        ...
        # =========================================================================== #
        ##### >>> 3 Train LSTM : 3 different seeds to see if the model stable >> consistency result 
        ...
        ### >> [Plot] Actual vs Predicted Price Over the History
        # Create figure
        fig = go.Figure()
...
df_metrics_all.to_csv(f"../../production/TEST/all_stocks_metrics.csv", index=False)
