...
## 1. Check for Run Number : 
## 2. Chcek the Batch Number :
## 2. Confirm target_col = [correct target] before commit the run 
...
exp_name = "NYSX_LSTM_RunX_predict_close"
...
for Tick in Batch_1: 
    output_PATH = f"../../production/Run_X/{Tick}"
    model_PATH =f"../../models/Run_X"
    os.makedirs(f"{output_PATH}", exist_ok=True)
    os.makedirs(f"{model_PATH}", exist_ok=True)
    
    # Artifact_PATH =f"../../production/Run/{Tick}"    
    # # Optional: see where the artifact will be stored
    # print(f"Artifact will be saved to:", {Artifact_PATH})
    
    RUN_NAME = f"RunX_{Tick}_LSTM_ver04.3"
    FILE_PATH = "../../data/raw"
    ...
    with mlflow.start_run(run_name=RUN_NAME, experiment_id=exp_id, nested = False):
        ...
        # =========================================================================== #
        ##### >>> 3 Train LSTM : 3 different seeds to see if the model stable <<< ##### 
